...
def create_input_queue(batch_size_per_clone, create_tensor_dict_fn,
  ...
  # Lidar
  tensor_dict[fields.InputDataFields.lidar] = tf.expand_dims(
                tensor_dict[fields.InputDataFields.lidar], 0)
  lidar_images = tensor_dict[fields.InputDataFields.lidar]
  float_lidar_images = tf.cast(lidar_images, dtype=tf.float32)
  tensor_dict[fields.InputDataFields.lidar] = float_lidar_images
...
def get_inputs(input_queue,
  ...
  def extract_images_and_targets(read_data):
    ...
    # lidar
    lidar = read_data[fields.InputDataFields.lidar]    
    key = ''
    ...
    # changes
    return (image, gated,lidar, key, location_gt, classes_gt, masks_gt, keypoints_gt,
            weights_gt)
...
def _create_losses(input_queue, create_model_fn, train_config):
  ...
  # changes
  (images, gated,lidar, _, groundtruth_boxes_list, groundtruth_classes_list,
  ...
  true_gated_shapes = []
  for image in gated:
    resized_gated, true_gated_shape = detection_model.preprocess(image)
    preprocessed_gated.append(resized_gated)
    true_gated_shapes.append(true_gated_shape)

  gated = tf.concat(preprocessed_gated, 0)
  true_gated_shapes = tf.concat(true_gated_shapes, 0)

  # lidar
  preprocessed_lidar = []
  true_lidar_shapes = []
  for image in lidar:
    resized_lidar, true_lidar_shape = detection_model.preprocess(image)
    preprocessed_lidar.append(resized_lidar)
    true_lidar_shapes.append(true_lidar_shape)

  lidar = tf.concat(preprocessed_lidar, 0)
  true_lidar_shapes = tf.concat(true_lidar_shapes, 0)
  ...
  # changes
  prediction_dict = detection_model.predict(images, gated, lidar, true_image_shapes, true_gated_shapes, true_lidar_shapes)
  # changes
  losses_dict = detection_model.loss(prediction_dict, true_image_shapes, true_gated_shapes, true_lidar_shapes)
