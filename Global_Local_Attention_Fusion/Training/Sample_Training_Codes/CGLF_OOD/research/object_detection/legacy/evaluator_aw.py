...
def _extract_predictions_and_losses(model,
  ...
  print("===========================Checkpoint Image=====================")
  print(original_gated)

  preprocessed_gated, true_gated_shapes = model.preprocess(
      tf.cast(original_gated, dtype=tf.float32))

  original_lidar = tf.expand_dims(input_dict[fields.InputDataFields.lidar], 0)
  preprocessed_lidar, true_lidar_shapes = model.preprocess(
      tf.cast(original_lidar, dtype=tf.float32))
  prediction_dict = model.predict(preprocessed_image,preprocessed_gated, preprocessed_lidar, true_image_shapes, true_gated_shapes, true_lidar_shapes)
  ...
  #result_dict = sess.run([prediction_dict])
    #result_dict, result_losses_dict = sess.run([result_dict, losses_dict])
  #  counters['success'] += 1
  ...
  if not ignore_groundtruth:
    ...
    losses_dict.update(model.loss(prediction_dict, true_image_shapes, true_gated_shapes, true_lidar_shapes))

  result_dict = eval_util.result_dict_for_single_example(
      original_image,prediction_dict['attention_weights_gated'],
  ...
  print("++++++++++++++++++Losses_Dict++++++++++++++++++++")
  print(losses_dict)
  #return result_dict, losses_dict, prediction_dict['rpn_features_to_crop']
...
def evaluate(create_input_dict_fn, create_model_fn, eval_config, categories,
             checkpoint_dir, eval_dir, graph_hook_fn=True, evaluator_list=None):
  ...
  def _process_batch(tensor_dict, sess, batch_index, counters, losses_dict=prediction_dict):
    ...
    try:
      ...
      #result_losses_dict, result_dict = sess.run([losses_dict, prediction_dict])
      counters['success'] += 1
    except tf.errors.InvalidArgumentError:
      logging.info('Skipping image')
      counters['skipped'] += 1
    ...
    #from PIL import Image
    data_camera = result_dict["key"]
    data_camera = data_camera[0,:,:,0]
#    data_camera = result_dict["attention_weights_camera"]
#    data_camera = data_camera[0,0,:,:,:]
#    data_gated = result_dict["attention_weights_camera"]
#    data_gated = data_gated[0,0,:,:,:]
    data_img = result_dict["original_image"]
    data_img = data_img[0,:,:,:3]
    print("Data Shape Camera:", data_camera.shape)
#    print("Data Shape Gated:", data_gated.shape)
    print("Data Image Shape", data_img.shape)
    #img = Image.fromarray(data_img, 'RGB')
    ax = sns.heatmap(data_camera, xticklabels=False, yticklabels=False, cbar=False)
    #ax = sns.heatmap(data_camera)
    plt.savefig("/home/saket/Dense/CGLF_OOD/research/object_detection/weights_gated.png", bbox_inches='tight', dpi=300)
    img = Image.fromarray(data_img, 'RGB')
    img.save('/home/saket/Dense/CGLF_OOD/research/object_detection/weights_original_image.png')
    #img = Image.fromarray(data_camera, 'RGB')
    #img.save('/home/saket/Dense/CLF_OOD/research/object_detection/weights_camera.png')
#    img = Image.fromarray(data_gated, 'RGB')
#    img.save('/home/saket/Dense/CLF_OOD/research/object_detection/weights_camera.png')
    print("Done")    
    ...
    print(result_dict)
    #print("+=+=+=+=+=+=++++++++++++Losses_Dict++++++++++++++++++++")
    #print(result_losses_dict)
    file = open("/home/saket/Dense/CGF_OOD/research/object_detection/Weights_Sample.txt", "w")
    file.write("%s = %s\n" %("Attention Weights", result_dict["key"]))
  ...
  # FirstStageFeatureExtractor/InceptionV2/Mixed_3c/Branch_1/Conv2d_0b_3x3
