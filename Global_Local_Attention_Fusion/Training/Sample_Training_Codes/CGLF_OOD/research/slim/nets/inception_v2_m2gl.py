  ...
  with tf.variable_scope(scope, 'InceptionV2', [inputs_camera, inputs_gated]):
    with slim.arg_scope(
      ...
      if include_root_block:
        ...
        # Upsampling
        net_camera = tf.keras.layers.UpSampling2D(size=(8,8))(net_camera)
        net_gated = tf.keras.layers.UpSampling2D(size=(8,8))(net_gated)
        ...
        # Camera Model
        # Local Attribute
        net_local_camera = slim.conv2d(inputs_camera, 48, [1, 1],stride=1)
    ...
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        ...
        weights_camera = tf.keras.layers.ZeroPadding2D(padding=((3,3),(0,0)))(weights_camera)
        #weights_camera = tf.nn.softmax(weights_camera)

        # Gated Model
        # Local Attribute
        net_local_gated = slim.conv2d(inputs_gated, 48, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
    ...
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        ...
        weights_gated = tf.keras.layers.ZeroPadding2D(padding=((3,3),(0,0)))(weights_gated)
        #weights_gated = tf.nn.softmax(weights_gated)
        
        weights = tf.concat(axis=concat_dim, values=[weights_camera, weights_gated])
        weights = tf.nn.softmax(weights, axis=concat_dim)

        #weights_camera = weights[:, :, :, 0:1] 
        #weights_gated = weights[:, :, :, 1:2]
        weights_camera, weights_gated = tf.split(weights, 2, axis=concat_dim)
      ...
      with tf.variable_scope(end_point):
        with tf.variable_scope('Branch_0'):
          branch_0_cam = slim.conv2d(net, depth(64), [1, 1], scope='Conv2d_0a_1x1')
        ...
        with tf.variable_scope('Branch_3'):
          branch_3_cam = slim.avg_pool2d(net, [3, 3], scope='AvgPool_0a_3x3')
      ...
      with tf.variable_scope(end_point):
        with tf.variable_scope('Branch_0'):
          branch_0_gated = slim.conv2d(net, depth(64), [1, 1], scope='Conv2d_0a_1x1')
        with tf.variable_scope('Branch_1'):
        ...
        with tf.variable_scope('Branch_2'):
          ...
          branch_2_gated = slim.conv2d(branch_2_gated, depth(96), [3, 3],
                                 scope='Conv2d_0c_3x3')
        with tf.variable_scope('Branch_3'):
          branch_3_gated = slim.avg_pool2d(net, [3, 3], scope='AvgPool_0a_3x3')
...
#        net = net_camera*weights_camera + net_gated*weights_gated
      ...
      with tf.variable_scope(end_point):
        ...
        net_gated = tf.concat(
            axis=concat_dim, values=[branch_0_gated, branch_1_gated, branch_2_gated, branch_3_gated])
        # Upsample
        #net_camera = tf.keras.layers.ZeroPadding2D(padding=((0,0),(1,1)))(net_camera)
        #net_gated = tf.keras.layers.ZeroPadding2D(padding=((0,0),(1,1)))(net_gated)
...
#        net = tf.concat(axis=concat_dim, values=[net_camera, net_gated])
        ...
        # Global Attribute
        #net_global = tf.reduce_mean(input_tensor=inputs, axis=[1, 2], keepdims=True)
#        net_global_camera = slim.avg_pool2d(inputs_camera, [184,170], stride=184)
#        net_global_camera = slim.conv2d(net_global_camera, 16, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
#        net_global_camera = slim.batch_norm(net_global_camera)
#        net_global_camera = tf.nn.relu(net_global_camera)
#        net_global_camera = slim.conv2d(net_global_camera, 16, [1, 1])
#        net_global_camera = slim.batch_norm(net_global_camera)
#        net_global_camera = tf.keras.layers.UpSampling2D(size=(184,171))(net_global_camera)
#        weights_camera = slim.conv2d(net_global_camera, 1, [1, 1])
        ...
        # Global Attribute
        #net_global = tf.reduce_mean(input_tensor=inputs, axis=[1, 2], keepdims=True)
#        net_global_gated = slim.avg_pool2d(inputs_gated, [184,170], stride=184)
#        net_global_gated = slim.conv2d(net_global_gated, 16, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
#        net_global_gated = slim.batch_norm(net_global_gated)
#        net_global_gated = tf.nn.relu(net_global_gated)
#        net_global_gated = slim.conv2d(net_global_gated, 16, [1, 1])
#        net_global_gated = slim.batch_norm(net_global_gated)
#        net_global_gated = tf.keras.layers.UpSampling2D(size=(184,171))(net_global_gated)
