  ...
  with tf.variable_scope(scope, 'InceptionV2', [inputs_camera, inputs_gated]):
    with slim.arg_scope(
      ...
      with tf.variable_scope(end_point):
        ...
        # Local Attribute
        net_local_c = slim.conv2d(inputs_c, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
        net_local_c = slim.batch_norm(net_local_c)
        net_local_c = slim.conv2d(net_local_c, 64, [1, 1], activation_fn=tf.nn.relu)
        net_local_c = slim.batch_norm(net_local_c)

        # Global Attribute
        # Global average pooling.
        #net_global = tf.reduce_mean(input_tensor=inputs, axis=[1, 2], keepdims=True)
        net_global_c = slim.avg_pool2d(inputs_c, [23,32], stride=34)
        net_global_c = slim.conv2d(net_global_c, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
        net_global_c = slim.batch_norm(net_global_c)
        net_global_c = slim.conv2d(net_global_c, 64, [1, 1], activation_fn=tf.nn.relu)
        net_global_c = slim.batch_norm(net_global_c)
        net_global_c = tf.keras.layers.UpSampling2D(size=(23,32))(net_global_c)

        # Combine Global Local Attributes and Calculating Weights
        net_c = net_local_c + net_global_c
        #net = tf.concat(axis=concat_dim, values=[net_global + net_local])
        weights_c = slim.conv2d(net_c, 1, [1, 1], activation_fn=tf.nn.sigmoid)
        

        # Local Attribute
        net_local_g = slim.conv2d(inputs_g, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
        net_local_g = slim.batch_norm(net_local_g)
        net_local_g = slim.conv2d(net_local_g, 64, [1, 1], activation_fn=tf.nn.relu)
        net_local_g = slim.batch_norm(net_local_g)

        # Global Attribute
        # Global average pooling.
        #net_global = tf.reduce_mean(input_tensor=inputs, axis=[1, 2], keepdims=True)
        net_global_g = slim.avg_pool2d(inputs_g, [23,32], stride=34)
        net_global_g = slim.conv2d(net_global_g, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
        net_global_g = slim.batch_norm(net_global_g)
        net_global_g = slim.conv2d(net_global_g, 64, [1, 1], activation_fn=tf.nn.relu)
        net_global_g = slim.batch_norm(net_global_g)
        net_global_g = tf.keras.layers.UpSampling2D(size=(23,32))(net_global_g)

        # Combine Global Local Attributes and Calculating Weights
        net_g = net_local_g + net_global_g
        #net = tf.concat(axis=concat_dim, values=[net_global + net_local])
        weights_g = slim.conv2d(net_g, 1, [1, 1], activation_fn=tf.nn.sigmoid)

        # Weighted Fusion
        #net = 2*branch_0 + 2*branch_1
        net = 2*net_camera*weights_c + 2*net_gated*weights_g
