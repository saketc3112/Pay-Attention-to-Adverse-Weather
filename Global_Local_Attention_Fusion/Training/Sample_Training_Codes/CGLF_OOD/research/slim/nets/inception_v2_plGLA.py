...
#        net = 2*net_camera*weights + 2*net_gated*(1 - weights)
    ...
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        ...
        # Global average pooling.
        #net_global = tf.reduce_mean(input_tensor=inputs, axis=[1, 2], keepdims=True)
        net_global = slim.avg_pool2d(inputs, [8,8], stride=8,  scope='Global_Network_1')
        net_global = slim.conv2d(net_global, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0), scope='Global_Network_2')
        net_global = slim.batch_norm(net_global, scope='Global_Network_3')
        net_global = slim.conv2d(net_global, 64, [1, 1], activation_fn=tf.nn.relu, scope='Global_Network_4')
    #net_global = slim.conv2d(net_global,depth(16), [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        net_global = slim.batch_norm(net_global, scope='Global_Network_5')
        net_global = tf.keras.layers.UpSampling2D(size=(7,8))(net_global)
        ...
        # Weighted Fusion
        #net = 2*branch_0 + 2*branch_1
        net_attention = 2*net_camera*weights + 2*net_gated*(1 - weights)
