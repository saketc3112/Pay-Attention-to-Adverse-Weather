  ...
  with tf.variable_scope(scope, 'InceptionV2', [inputs_camera, inputs_gated]):
    with slim.arg_scope(
      ...
      if include_root_block:
        ...
        # Upsampling
        #net_camera = tf.keras.layers.UpSampling2D(size=(8,8))(net_camera_common)
        #net_gated = tf.keras.layers.UpSampling2D(size=(8,8))(net_gated_common)
        ...
        # Camera Model
        # Local Attribute
        net_local_camera = slim.conv2d(net_camera_common, 48, [1, 1],stride=1)
    ...
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        net_local_camera = slim.batch_norm(net_local_camera)
        #net_local_camera = tf.keras.layers.UpSampling2D(size=(8,8))(net_local_camera)
        weights_local_camera = slim.conv2d(net_local_camera, 1, [1, 1])
        #weights_local_camera = tf.keras.layers.ZeroPadding2D(padding=((3,3),(0,0)))(weights_local_camera)
        #weights_local_camera = tf.nn.sigmoid(weights_local_camera)

        # Gated Model
        # Local Attribute
        net_local_gated = slim.conv2d(net_gated_common, 48, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
    ...
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        net_local_gated = slim.batch_norm(net_local_gated)
        #net_local_gated = tf.keras.layers.UpSampling2D(size=(8,8))(net_local_gated)
        weights_local_gated = slim.conv2d(net_local_gated, 1, [1, 1])
        #weights_local_gated = tf.keras.layers.ZeroPadding2D(padding=((3,3),(0,0)))(weights_local_gated)
        #weights_local_gated = tf.nn.sigmoid(weights_local_gated)
        
        weights = tf.concat(axis=concat_dim, values=[weights_local_camera, weights_local_gated])
        weights = tf.nn.softmax(weights, axis=concat_dim)

        #weights_camera = weights[:, :, :, 0:1] 
        #weights_gated = weights[:, :, :, 1:2]
        weights_local_camera, weights_local_gated = tf.split(weights, 2, axis=concat_dim)

        net_local = net_camera_common*weights_local_camera + net_gated_common*weights_local_gated
...
#        net = net_camera*weights_camera + net_gated*weights_gated
      ...
      with tf.variable_scope(end_point):
        ...
        net_gated = tf.concat(
            axis=concat_dim, values=[branch_0_gated, branch_1_gated, branch_2_gated, branch_3_gated])
        # Upsample
        #net_camera = tf.keras.layers.ZeroPadding2D(padding=((3,3),(1,1)))(net_camera)
        #net_gated = tf.keras.layers.ZeroPadding2D(padding=((3,3),(1,1)))(net_gated)
...
#        net = tf.concat(axis=concat_dim, values=[net_camera, net_gated])
        ...
        # Global Attribute
        #net_global = tf.reduce_mean(input_tensor=inputs, axis=[1, 2], keepdims=True)
        #net_global_camera = slim.avg_pool2d(inputs_camera, [92,115], stride=100)
        ...
        #net_global_camera = tf.keras.layers.UpSampling2D(size=(12,12))(net_global_camera)
        weights_global_camera = slim.conv2d(net_global_camera, 1, [1, 1])
        #weights_global_camera = tf.nn.sigmoid(weights_global_camera)

        # Gated
        # Global Attribute
        #net_global = tf.reduce_mean(input_tensor=inputs, axis=[1, 2], keepdims=True)
        #net_global_gated = slim.avg_pool2d(inputs_gated, [92,115], stride=100)
        ...
        #net_global_gated = tf.keras.layers.UpSampling2D(size=(12,12))(net_global_gated)
        weights_global_gated = slim.conv2d(net_global_gated, 1, [1, 1])
        #weights_global_gated = tf.nn.softmax(weights_global_gated)

        weights = tf.concat(axis=concat_dim, values=[weights_global_camera, weights_global_gated])
        weights = tf.nn.softmax(weights, axis=concat_dim)

        #weights_camera = weights[:, :, :, 0:1] 
        #weights_gated = weights[:, :, :, 1:2]
        weights_global_camera, weights_global_gated = tf.split(weights, 2, axis=concat_dim)

        net_global = net_camera*weights_global_camera + net_gated*weights_global_gated
        # Upsampling
        #net_global = tf.keras.layers.UpSampling2D(size=(8,8))(net_global)
      ...
      with tf.variable_scope(end_point):
        ...
        net = tf.concat(
            axis=concat_dim, values=[branch_0, branch_1, branch_2, branch_3])
        end_points[end_point] = net
        if end_point == final_endpoint: return net, end_points, weights_global_camera, weights_global_gated
      # 14 x 14 x 576
      end_point = 'Mixed_4e'
      with tf.variable_scope(end_point):
        with tf.variable_scope('Branch_0'):
