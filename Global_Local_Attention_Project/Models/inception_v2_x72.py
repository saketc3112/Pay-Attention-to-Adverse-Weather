  ...
  with tf.variable_scope(scope, 'InceptionV2', [inputs_camera, inputs_gated, inputs_lidar]):
    with slim.arg_scope(
      ...
      if include_root_block:
        ...
        # Camera Model
        # Local Attribute
        net_local_camera = slim.conv2d(in_camera, 48, [1, 1],stride=1, weights_initializer=trunc_normal(1.0))
        net_local_camera = slim.batch_norm(net_local_camera)
        net_local_camera = tf.nn.relu(net_local_camera)
        net_local_camera = slim.conv2d(net_local_camera, 48, [1, 1])
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        net_local_camera = slim.batch_norm(net_local_camera)
        weights_camera = slim.conv2d(net_local_camera, 1, [1, 1])
        #weights_camera_local = tf.nn.sigmoid(weights_camera)

        # Gated Model
        # Local Attribute
        net_local_gated = slim.conv2d(in_gated, 48, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
        net_local_gated = slim.batch_norm(net_local_gated)
        net_local_gated = tf.nn.relu(net_local_gated)
        net_local_gated = slim.conv2d(net_local_gated, 48, [1, 1])
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        net_local_gated = slim.batch_norm(net_local_gated)
        weights_gated = slim.conv2d(net_local_gated, 1, [1, 1])
        #weights_gated_local = tf.nn.sigmoid(weights_gated)
        
        # Lidar Model
        # Local Attribute
        net_local_lidar = slim.conv2d(in_lidar, 48, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
        net_local_lidar = slim.batch_norm(net_local_lidar)
        net_local_lidar = tf.nn.relu(net_local_lidar)
        net_local_lidar = slim.conv2d(net_local_lidar, 48, [1, 1])
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        net_local_lidar = slim.batch_norm(net_local_lidar)
        weights_lidar = slim.conv2d(net_local_lidar, 1, [1, 1])
        #weights_lidar_local = tf.nn.sigmoid(weights_lidar)

        weights_local = tf.concat(axis=concat_dim, values=[weights_camera, weights_gated, weights_lidar])
        weights_local = tf.nn.softmax(weights_local, axis=concat_dim)

        #weights_camera = weights[:, :, :, 0:1] 
        #weights_gated = weights[:, :, :, 1:2]
        weights_camera_local, weights_gated_local, weights_lidar_local = tf.split(weights_local, 3, axis=concat_dim)

        net_local = net_camera_common*weights_camera_local + net_gated_common*weights_gated_local + net_lidar_common*weights_lidar_local
        # Upsampling
        #net_local = tf.keras.layers.ZeroPadding2D(padding=((1,0),(1,1)))(net_local)

        end_points[end_point] = net_local
        if end_point == final_endpoint:
          return net_local, end_points
      ...
      with tf.variable_scope(end_point):
        with tf.variable_scope('Branch_0'):
          branch_0_cam = slim.conv2d(net_local, depth(64), [1, 1], scope='Conv2d_0a_1x1')
        with tf.variable_scope('Branch_1'):
          branch_1_cam = slim.conv2d(
              net_local, depth(64), [1, 1],
        ...
        with tf.variable_scope('Branch_3'):
          branch_3_cam = slim.avg_pool2d(net_local, [3, 3], scope='AvgPool_0a_3x3')
      ...
      with tf.variable_scope(end_point):
        with tf.variable_scope('Branch_0'):
          branch_0_gated = slim.conv2d(net_local, depth(64), [1, 1], scope='Conv2d_0a_1x1')
        with tf.variable_scope('Branch_1'):
        ...
        with tf.variable_scope('Branch_2'):
          ...
          branch_2_gated = slim.conv2d(branch_2_gated, depth(96), [3, 3],
                                 scope='Conv2d_0c_3x3')
        with tf.variable_scope('Branch_3'):
          branch_3_gated = slim.avg_pool2d(net_local, [3, 3], scope='AvgPool_0a_3x3')
      ...
      with tf.variable_scope(end_point):
        with tf.variable_scope('Branch_0'):
          branch_0_lidar = slim.conv2d(net_local, depth(64), [1, 1], scope='Conv2d_0a_1x1')
        with tf.variable_scope('Branch_1'):
        ...
        with tf.variable_scope('Branch_2'):
          ...
          branch_2_lidar = slim.conv2d(branch_2_lidar, depth(96), [3, 3],
                                 scope='Conv2d_0c_3x3')
        with tf.variable_scope('Branch_3'):
          branch_3_lidar = slim.avg_pool2d(net_local, [3, 3], scope='AvgPool_0a_3x3')
...
#        net = net_camera*weights_camera + net_gated*weights_gated
      ...
      with tf.variable_scope(end_point):
        ...
        # Upsampling
        in_camera = tf.keras.layers.ZeroPadding2D(padding=((0,0),(3,2)))(net_camera)
        in_gated = tf.keras.layers.ZeroPadding2D(padding=((0,0),(3,2)))(net_gated)
        in_lidar = tf.keras.layers.ZeroPadding2D(padding=((0,0),(3,2)))(net_lidar)

        # Upsampling
        net_camera = tf.keras.layers.ZeroPadding2D(padding=((2,1),(2,2)))(net_camera)
        net_gated = tf.keras.layers.ZeroPadding2D(padding=((2,1),(2,2)))(net_gated)
        net_lidar = tf.keras.layers.ZeroPadding2D(padding=((2,1),(2,2)))(net_lidar)

        # Camera
        # Global Attribute
        #net_global = tf.reduce_mean(input_tensor=inputs, axis=[1, 2], keepdims=True)
        net_global_camera = slim.avg_pool2d(in_camera, [11,11], stride=12)
        net_global_camera = slim.conv2d(net_global_camera, 16, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
        net_global_camera = slim.batch_norm(net_global_camera)
        net_global_camera = tf.nn.relu(net_global_camera)
        net_global_camera = slim.conv2d(net_global_camera, 16, [1, 1])
        net_global_camera = slim.batch_norm(net_global_camera)
        net_global_camera = tf.keras.layers.UpSampling2D(size=(12,11))(net_global_camera)
        ...
        # Global Attribute
        #net_global = tf.reduce_mean(input_tensor=inputs, axis=[1, 2], keepdims=True)
        net_global_gated = slim.avg_pool2d(in_gated, [11,11], stride=12)
        net_global_gated = slim.conv2d(net_global_gated, 16, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
        net_global_gated = slim.batch_norm(net_global_gated)
        net_global_gated = tf.nn.relu(net_global_gated)
        net_global_gated = slim.conv2d(net_global_gated, 16, [1, 1])
        net_global_gated = slim.batch_norm(net_global_gated)
        net_global_gated = tf.keras.layers.UpSampling2D(size=(12,11))(net_global_gated)
        ...
        # Global Attribute
        #net_global = tf.reduce_mean(input_tensor=inputs, axis=[1, 2], keepdims=True)
        net_global_lidar = slim.avg_pool2d(in_lidar, [11,11], stride=12)
        ...
        net_global_lidar = slim.batch_norm(net_global_lidar)
        net_global_lidar = tf.keras.layers.UpSampling2D(size=(12,11))(net_global_lidar)
      ...
      with tf.variable_scope(end_point):
        ...
        net = tf.concat(
            axis=concat_dim, values=[branch_0, branch_1, branch_2, branch_3])
        end_points[end_point] = net
        if end_point == final_endpoint: return net, end_points, weights_camera_global, weights_gated_global, weights_lidar_global
      # 14 x 14 x 576
      end_point = 'Mixed_4e'
      with tf.variable_scope(end_point):
        with tf.variable_scope('Branch_0'):
