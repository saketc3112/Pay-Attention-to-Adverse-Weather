...
def inception_v2_base(inputs_camera, inputs_gated, inputs_lidar, 
  ...
  with tf.variable_scope(scope, 'InceptionV2', [inputs_camera, inputs_gated, inputs_lidar]):
    ...
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        ...
        weights_local = tf.concat(axis=concat_dim, values=[weights_camera, weights_gated])
        weights_local = tf.nn.softmax(weights_local, axis=concat_dim)

        #weights_camera = weights[:, :, :, 0:1] 
        #weights_gated = weights[:, :, :, 1:2]
        weights_camera_local, weights_gated_local = tf.split(weights_local, 2, axis=concat_dim)

        net_local = net_camera_common*weights_camera_local + net_gated_common*weights_gated_local
        # Upsampling
      ...
      with tf.variable_scope(end_point):
        ...
        weights_global = tf.concat(axis=concat_dim, values=[weights_camera_global, weights_gated_global])
        weights_global = tf.nn.softmax(weights_global, axis=concat_dim)

        #weights_camera = weights[:, :, :, 0:1] 
        #weights_gated = weights[:, :, :, 1:2]
        weights_camera_global, weights_gated_global = tf.split(weights_global, 2, axis=concat_dim)

        net_global = net_camera*weights_camera_global + net_gated*weights_gated_global
      ...
      with tf.variable_scope(end_point):
        ...
        net = tf.concat(
            axis=concat_dim, values=[branch_0, branch_1, branch_2, branch_3])
        end_points[end_point] = net
        if end_point == final_endpoint: return net, end_points, weights_camera_global, weights_gated_global, weights_gated_global
      # 14 x 14 x 576
      end_point = 'Mixed_4e'
      with tf.variable_scope(end_point):
        with tf.variable_scope('Branch_0'):
