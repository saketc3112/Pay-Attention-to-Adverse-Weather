...
class ExampleCreator(object):
    ...
    def proces_label(self, entry_id, image_shape):
        ...
        o = self.process_fclass(o)
        #o = self.process_flabel(o)
        o = self.process_fxmin(o)
        o = self.process_fymin(o)
        o = self.process_fxmax(o)
        o = self.process_fymax(o)
        o = self.process_le(o)
        o = self.process_flabel(o)
        return o
    ...
    def process_le(self, parsed_labels):
        """
        :return: label object with filled out labels value
        """
        lal = []
        for i in parsed_labels.classes:
            ...
            if i == b'DontCare':
                lal.append(0)
        parsed_labels.lal = lal
        print('Equi Labels:', parsed_labels.lal)
        return parsed_labels
    ...
    def process_fclass(self, parsed_labels):
        """
        :return: label object with filled out labels value
        """
        fclass_ = []
        for idx,xm in enumerate(parsed_labels.difficults):
            #for i in parsed_labels.xmin:
            #if (parsed_labels.ca[idx] == b'Vehicle' or parsed_labels.ca[idx] == b'DontCare') and (xm == 0):
            if (parsed_labels.classes[idx] == b'Pedestrian' or parsed_labels.classes[idx] == b'person' or parsed_labels.classes[idx] == b'RidableVehicle'  or parsed_labels.classes[idx] == b'PassengerCar' or parsed_labels.classes[idx] == b'LargeVehicle' or parsed_labels.classes[idx] == b'train' or parsed_labels.classes[idx] == b'DontCare') and (xm == 0):
                fclass_.append(parsed_labels.ca[idx])
        parsed_labels.fclass_ = fclass_
        print('Filtered Easy Classes:', parsed_labels.fclass_)
        return parsed_labels

    def process_flabel(self, parsed_labels):
        """
        :return: label object with filled out labels value
        """
        flabel_ = []
        for idx,xm in enumerate(parsed_labels.difficults):
            #fl_ = p_label(parsed_labels.ca[idx])
            #for i in parsed_labels.xmin:
            #if (parsed_labels.la[idx] == 0 or parsed_labels.la[idx] == 1) and (xm == 0):
            if (parsed_labels.classes[idx] == b'Pedestrian' or parsed_labels.classes[idx] == b'person' or parsed_labels.classes[idx] == b'RidableVehicle'  or parsed_labels.classes[idx] == b'PassengerCar' or parsed_labels.classes[idx] == b'LargeVehicle' or parsed_labels.classes[idx] == b'train' or parsed_labels.classes[idx] == b'DontCare') and (xm == 0):
                flabel_.append(parsed_labels.lal[idx])
        parsed_labels.flabel_ = flabel_
        print('Filtered Easy Labels:', parsed_labels.flabel_)
        return parsed_labels

    def process_fxmin(self, parsed_labels):
        """
        :return: label object with filled out labels value
        """
        fxmin_ = []
        for idx,xm in enumerate(parsed_labels.difficults):
            #for i in parsed_labels.xmin:
            if (parsed_labels.classes[idx] == b'Pedestrian' or parsed_labels.classes[idx] == b'person' or parsed_labels.classes[idx] == b'RidableVehicle'  or parsed_labels.classes[idx] == b'PassengerCar' or parsed_labels.classes[idx] == b'LargeVehicle' or parsed_labels.classes[idx] == b'train' or parsed_labels.classes[idx] == b'DontCare') and (xm == 0):
                fxmin_.append(parsed_labels.xmin[idx])
        parsed_labels.fxmin_ = fxmin_
        print('Filtered Easy Xmin:', parsed_labels.fxmin_)
        return parsed_labels

    def process_fymin(self, parsed_labels):
        """
        :return: label object with filled out labels value
        """
        fymin_ = []
        for idx,xm in enumerate(parsed_labels.difficults):
            #for i in parsed_labels.xmin:
            if (parsed_labels.classes[idx] == b'Pedestrian' or parsed_labels.classes[idx] == b'person' or parsed_labels.classes[idx] == b'RidableVehicle'  or parsed_labels.classes[idx] == b'PassengerCar' or parsed_labels.classes[idx] == b'LargeVehicle' or parsed_labels.classes[idx] == b'train' or parsed_labels.classes[idx] == b'DontCare') and (xm == 0):
                fymin_.append(parsed_labels.ymin[idx])
        parsed_labels.fymin_ = fymin_
        print('Filtered Easy Ymin:', parsed_labels.fymin_)
        return parsed_labels

    def process_fxmax(self, parsed_labels):
        """
        :return: label object with filled out labels value
        """
        fxmax_ = []
        for idx,xm in enumerate(parsed_labels.difficults):
            #for i in parsed_labels.xmin:
            if (parsed_labels.classes[idx] == b'Pedestrian' or parsed_labels.classes[idx] == b'person' or parsed_labels.classes[idx] == b'RidableVehicle'  or parsed_labels.classes[idx] == b'PassengerCar' or parsed_labels.classes[idx] == b'LargeVehicle' or parsed_labels.classes[idx] == b'train' or parsed_labels.classes[idx] == b'DontCare') and (xm == 0):
                fxmax_.append(parsed_labels.xmax[idx])
        parsed_labels.fxmax_ = fxmax_
        print('Filtered Easy Xmax:', parsed_labels.fxmax_)
        return parsed_labels

    def process_fymax(self, parsed_labels):
        """
        :return: label object with filled out labels value
        """
        fymax_ = []
        for idx,xm in enumerate(parsed_labels.difficults):
            #for i in parsed_labels.xmin:
            if (parsed_labels.classes[idx] == b'Pedestrian' or parsed_labels.classes[idx] == b'person' or parsed_labels.classes[idx] == b'RidableVehicle'  or parsed_labels.classes[idx] == b'PassengerCar' or parsed_labels.classes[idx] == b'LargeVehicle' or parsed_labels.classes[idx] == b'train' or parsed_labels.classes[idx] == b'DontCare') and (xm == 0):
                fymax_.append(parsed_labels.ymax[idx])
        parsed_labels.fymax_ = fymax_
        print('Filtered Easy Ymax:', parsed_labels.fymax_)
        return parsed_labels
...
class SwedenImagesv2(ExampleCreator):
    ...
    def create_example(self, data):
        ...
        print("Filtered Class length:", len(label.fclass_))
        print("Class BoundingBox:",len(label.xmin))
        print("Filtered BoundingBox length:",len(label.fxmin_))

        if len(label.fclass_) != len(label.flabel_):
            print("Danger")
        if len(label.fclass_) == 0:
            print("Empty")
        # Easy Instances
        count = 0
        for i in label.difficults:
            if i == 1 or 2:
                count = count + 1
        if count == 0:
            print("Easy Instance")
        ...
        feature_dict = {
            ...
            'image/object/class/text': bytes_feature(label.fclass_),
            'image/object/class/label': int64_feature(label.flabel_),
            'image/object/bbox/xmin': float_feature(label.fxmin_),
            'image/object/bbox/xmax': float_feature(label.fxmax_),
            'image/object/bbox/ymin': float_feature(label.fymin_),
            'image/object/bbox/ymax': float_feature(label.fymax_),
