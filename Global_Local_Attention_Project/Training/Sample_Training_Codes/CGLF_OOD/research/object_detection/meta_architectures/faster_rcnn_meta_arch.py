...
class FasterRCNNFeatureExtractor(object):
  ...
  # changes
  def extract_proposal_features(self, preprocessed_inputs,preprocessed_gated,preprocessed_lidar, scope):
    ...
    """
    with tf.variable_scope(scope, values=[preprocessed_inputs,preprocessed_gated, preprocessed_lidar]):
      return self._extract_proposal_features(preprocessed_inputs, preprocessed_gated,preprocessed_lidar, scope)

  @abc.abstractmethod
  def _extract_proposal_features(self, preprocessed_inputs, preprocessed_gated, preprocessed_lidar, scope):
...
class FasterRCNNMetaArch(model.DetectionModel):
  ...
  def predict(self, preprocessed_inputs, preprocessed_gated, preprocessed_lidar, true_image_shapes, true_gated_shapes, true_lidar_shapes, **side_inputs):
    ...
    """
    prediction_dict = self._predict_first_stage(preprocessed_inputs,preprocessed_gated, preprocessed_lidar)
    ...
    if self._number_of_stages >= 2:
      prediction_dict.update(
          self._predict_second_stage(
              ...
              true_image_shapes, true_gated_shapes, true_lidar_shapes,
              **side_inputs))
  ...
  def _predict_first_stage(self, preprocessed_inputs,preprocessed_gated, preprocessed_lidar):
    ...
    (rpn_box_predictor_features, rpn_features_to_crop, anchors_boxlist,
     image_shape, attention_weights_camera, attention_weights_gated, attention_weights_lidar) = self._extract_rpn_feature_maps(preprocessed_inputs, preprocessed_gated, preprocessed_lidar)
    (rpn_box_encodings, rpn_objectness_predictions_with_background
    ...
    prediction_dict = {
        ...
        'attention_weights_camera':
            attention_weights_camera,
        'attention_weights_gated':
            attention_weights_gated,
        'attention_weights_lidar':
            attention_weights_lidar
    }
    return prediction_dict
  ...
  def _predict_second_stage(self, rpn_box_encodings,
                            ...
                            true_image_shapes, true_gated_shapes, true_lidar_shapes, **side_inputs):
  ...
  def _predict_third_stage(self, prediction_dict, image_shapes, gated_shapes, lidar_shapes):
  ...
  def _extract_rpn_feature_maps(self, preprocessed_inputs, preprocessed_gated, preprocessed_lidar):
    ...
    #    preprocessed_inputs, preprocessed_gated, preprocessed_lidar)
    rpn_features_to_crop, self.endpoints, self.attention_weights_camera, self.attention_weights_gated, self.attention_weights_lidar = self._extract_proposal_features(preprocessed_inputs, preprocessed_gated, preprocessed_lidar)
    ...
    #        anchors, image_shape)
    return (rpn_box_predictor_features, rpn_features_to_crop,
            anchors, image_shape, self.attention_weights_camera, self.attention_weights_gated, self.attention_weights_lidar)

  def _extract_proposal_features(self, preprocessed_inputs, preprocessed_gated, preprocessed_lidar):
  ...
  def loss(self, prediction_dict, true_image_shapes, true_gated_shapes,true_lidar_shapes, scope=None):
