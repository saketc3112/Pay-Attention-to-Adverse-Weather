...
def inception_v2_base(inputs_camera, inputs_gated, inputs_lidar,
  ...
  with tf.variable_scope(scope, 'InceptionV2', [inputs_camera, inputs_gated, inputs_lidar]):
    ...
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        ...
        weights = tf.concat(axis=concat_dim, values=[weights_camera, weights_gated, weights_lidar])
        ...
        weights_camera, weights_gated, weights_lidar = tf.split(weights, 3, axis=concat_dim)

        net_local = net_camera*weights_camera + net_gated*weights_gated + net_lidar*weights_lidar
...
#        net = net_camera*weights_camera + net_gated*weights_gated
      ...
      with tf.variable_scope(end_point):
        ...
        weights_camera = slim.conv2d(net_global_camera, 1, [1, 1])
        #weights_gated = tf.nn.softmax(weights_gated)
        ...
        weights_gated = slim.conv2d(net_global_gated, 1, [1, 1])
        #weights_gated = tf.nn.softmax(weights_gated)
        ...
        weights_lidar = slim.conv2d(net_global_lidar, 1, [1, 1])
        #weights_gated = tf.nn.softmax(weights_gated)
        ...
        #weights_camera = weights[:, :, :, 0:1] 
        #weights_gated = weights[:, :, :, 1:2]
        weights_camera_global, weights_gated_global, weights_lidar_global = tf.split(weights, 3, axis=concat_dim)
