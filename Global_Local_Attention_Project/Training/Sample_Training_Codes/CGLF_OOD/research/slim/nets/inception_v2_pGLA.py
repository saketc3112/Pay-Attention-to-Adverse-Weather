  ...
  with tf.variable_scope(scope, 'InceptionV2', [inputs_camera, inputs_gated]):
    with slim.arg_scope(
      ...
      with tf.variable_scope(end_point):
        ...
        # Local Attribute
        net_local = slim.conv2d(inputs, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
        net_local = slim.batch_norm(net_local)
        net_local = slim.conv2d(net_local, 64, [1, 1], activation_fn=tf.nn.relu)
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        net_local = slim.batch_norm(net_local)
        ...
        net_global = slim.avg_pool2d(inputs, [23,32], stride=34)
        net_global = slim.conv2d(net_global, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0))
        net_global = slim.batch_norm(net_global)
        net_global = slim.conv2d(net_global, 64, [1, 1], activation_fn=tf.nn.relu)
    #net_global = slim.conv2d(net_global,depth(16), [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        net_global = slim.batch_norm(net_global)
