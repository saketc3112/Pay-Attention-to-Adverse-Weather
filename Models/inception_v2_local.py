...
def inception_v2_base(inputs_camera, inputs_gated, inputs_lidar, 
  ...
  with tf.variable_scope(scope, 'InceptionV2', [inputs_camera, inputs_gated, inputs_lidar]):
    ...
    #net_local = slim.conv2d(net_local, 64, [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
        ...
        # Upsampling
        net_camera_common = tf.keras.layers.ZeroPadding2D(padding=((1,0),(1,1)))(net_local)

        end_points[end_point] = net_camera_common
        if end_point == final_endpoint:
          return net_camera_common, end_points
...
#        net = net_camera*weights_camera + net_gated*weights_gated
      ...
      with tf.variable_scope(end_point):
        ...
        with tf.variable_scope('Branch_3'):
          ...
          branch_3_cam = slim.conv2d(
              ...
              weights_initializer=trunc_normal(0.1),
              scope='Conv2d_0b_1x1')
        net_camera = tf.concat(axis=concat_dim, values=[branch_0_cam, branch_1_cam, branch_2_cam, branch_3_cam])
        net = net_camera
      ...
      with tf.variable_scope(end_point):
        ...
        net = tf.concat(
            axis=concat_dim, values=[branch_0, branch_1, branch_2, branch_3])
        end_points[end_point] = net
        if end_point == final_endpoint: return net, end_points, weights_camera_local, weights_gated_local, weights_lidar_local
      # 14 x 14 x 576
      end_point = 'Mixed_4e'
      with tf.variable_scope(end_point):
        with tf.variable_scope('Branch_0'):
