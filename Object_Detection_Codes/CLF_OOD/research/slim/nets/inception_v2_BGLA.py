...
class PredictiveEntropy(tf.keras.layers.Layer):
...
def inception_v2_base(inputs_camera, inputs_gated,
  ...
  """Inception v2 Bayesian Neural Network. This method can construct the network up to the layer inception(5b) as described in http://arxiv.org/abs/1502.03167.
  ...
  with tf.variable_scope(scope, 'InceptionV2', [inputs_camera, inputs_gated]):
    with slim.arg_scope(
      ...
      if include_root_block:
        ...
        net_camera = slim.max_pool2d(net_camera, [3, 3], scope=end_point, stride=2)
        # MC Droupout
        net_camera = apply_dropout(net_camera, dropout_rate, filterwise_dropout)
        # Softmax Output
        softmax_logits_1_camera = slim.conv2d(net_camera, 1, [1, 1], activation_fn=tf.nn.sigmoid,
                             normalizer_fn=None)
        
        # Camera Model: Block 1 MC 2
        net_camera = slim.separable_conv2d(
              inputs_camera,
              depth(64), [7, 7],
              depth_multiplier=depthwise_multiplier,
        ...
        net_camera = apply_dropout(net_camera, dropout_rate, filterwise_dropout)
        # Softmax Output
        softmax_logits_2_camera = slim.conv2d(net_camera, 1, [1, 1], activation_fn=tf.nn.sigmoid,
                             normalizer_fn=None)
        
        # Camera Model: Block 1 MC 3
        net_camera = slim.separable_conv2d(
              inputs_camera,
              depth(64), [7, 7],
              depth_multiplier=depthwise_multiplier,
        ...
        net_camera = apply_dropout(net_camera, dropout_rate, filterwise_dropout)
        # Softmax Output
        softmax_logits_3_camera = slim.conv2d(net_camera, 1, [1, 1], activation_fn=tf.nn.sigmoid,
                             normalizer_fn=None)
        # Uncertainty Weights
        softmax_mean_camera = (softmax_logits_1_camera + softmax_logits_2_camera + softmax_logits_3_camera)/3
        # variance uncertainty
        camera_uncertainty = PredictiveEntropy(softmax_mean_camera)

        # Gated Model: Block 1 MC 1
        ...
        net_gated = slim.max_pool2d(net_gated, [3, 3], scope=end_point, stride=2)
        # MC Droupout
        net_gated = apply_dropout(net_gated, dropout_rate, filterwise_dropout)
        # Softmax Output
        softmax_logits_1_gated = slim.conv2d(net_gated, 1, [1, 1], activation_fn=tf.nn.sigmoid,
                             normalizer_fn=None)

        # Gated Model: Block 1 MC 2
        net_gated = slim.separable_conv2d(
              inputs_gated,
              depth(64), [7, 7],
              depth_multiplier=depthwise_multiplier,
        ...
        net_gated = apply_dropout(net_gated, dropout_rate, filterwise_dropout)
        # Softmax Output
        softmax_logits_2_gated = slim.conv2d(net_gated, 1, [1, 1], activation_fn=tf.nn.sigmoid,
                             normalizer_fn=None)

        # Gated Model: Block 1 MC 3
        net_gated = slim.separable_conv2d(
              inputs_gated,
              depth(64), [7, 7],
              depth_multiplier=depthwise_multiplier,
        ...
        net_gated = apply_dropout(net_gated, dropout_rate, filterwise_dropout)
        # Softmax Output
        softmax_logits_3_gated = slim.conv2d(net_gated, 1, [1, 1], activation_fn=tf.nn.sigmoid,
                             normalizer_fn=None)
        
        # Uncertainty Weights
        softmax_mean_gated = (softmax_logits_1_gated + softmax_logits_2_gated + softmax_logits_3_gated)/3
        # variance uncertainty
        gated_uncertainty = PredictiveEntropy(softmax_mean_gated)
      ...
      # Weighted Bayesian Uncertainty Fusion
      #inputs = camera_uncertainty*net_camera + gated_uncertainty*net_gated
      #inputs = 2*camera_uncertainty*net_camera + 2*(1 - camera_uncertainty)*net_gated
      #inputs = net_camera + net_gated
      inputs = 2*net_camera*softmax_mean_camera + 2*net_gated*(1 - softmax_mean_gated)

      # Local Attribute
      net_local = slim.conv2d(inputs, depth(16), [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
      net_local = slim.batch_norm(net_local)
      net_local = slim.conv2d(net_local, depth(16), [1, 1], activation_fn=tf.nn.relu)
      #net_local = slim.conv2d(net_local,depth(64), [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
      net_local = slim.batch_norm(net_local)
      ...
      net_global = slim.conv2d(net_global, depth(16), [1, 1], activation_fn=tf.nn.relu)
      #net_global = slim.conv2d(net_global,depth(16), [1, 1],stride=1,weights_initializer=trunc_normal(1.0),scope=end_point)
      net_global = slim.batch_norm(net_global)

      # Combine Global Local Attributes and Calculating Weights
      #net = net_global + net_local
      weights = slim.conv2d(net_local, 1, [1, 1], activation_fn=tf.nn.sigmoid)
