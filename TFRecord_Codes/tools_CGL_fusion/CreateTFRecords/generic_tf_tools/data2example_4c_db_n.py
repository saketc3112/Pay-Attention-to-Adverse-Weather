...
def entropy_image(sk_image, entropy_path):
    sk_image = sk_image.transpose(1,0,2).reshape(1920,-1)
    ...
    ax1.invert_yaxis()
    plt.axis('off')
    plt.savefig(entropy_path, bbox_inches='tight', pad_inches = 0)
    image = Image.open(entropy_path)
    newsize = (1920, 1024)
    image = image.resize(newsize)
    image = image.save(entropy_path)
    return img1

def entropy_image_g(sk_image, entropy_path):
    sk_image = sk_image.transpose(1,0).reshape(1280,-1)
    image = img_as_ubyte(sk_image)
    image = ndimage.rotate(image, 90)
    fig, ax1 = plt.subplots(dpi=400)
    img1 = ax1.imshow(entropy(image, disk(5)), cmap='gray')
...
def lidar_points_image(points,
    ...
    newsize = (1920, 1024)
    image = image.resize(newsize)
    image = image.save(saveto)
    feature = open(saveto, 'rb').read()
    
    file_path = saveto
    sk_image = imread(file_path) #read the image to convert to skimage ndarray
    entropy_path = "/home/saket/Dense/entropy_sample_lidar.png"
    im1 = entropy_image(sk_image, entropy_path)
    feature_entropy = open(entropy_path, 'rb').read()

    return feature, feature_entropy
...
class ExampleCreator(object):
    ...
    def process_occ(self, parsed_labels):
        ...
        for idx,xm in enumerate(parsed_labels.occlusion):
            ...
            if parsed_labels.classes[idx] == b'Pedestrian' or parsed_labels.classes[idx] == b'RidableVehicle'  or parsed_labels.classes[idx] == b'PassengerCar' or parsed_labels.classes[idx] == b'LargeVehicle':
                occ_.append(xm)
    ...
    def process_xmin(self, parsed_labels):
        ...
        for idx,xm in enumerate(parsed_labels.xmin):
            #for i in parsed_labels.xmin:
            if parsed_labels.classes[idx] == b'Pedestrian':
                xmin_.append(xm)
...
class SwedenImagesv2(ExampleCreator):
    ...
    def read_data(self, entry_id, total_id):
        ...
        for folder in self.image_keys:
            ...
            sk_image = imread(file_path) #read the image to convert to skimage ndarray
            entropy_path = "/home/saket/Dense/entropy_sample_camera.png"
            im1 = entropy_image(sk_image, entropy_path)
            feature_entropy = open(entropy_path, 'rb').read()
            dist_images_entropy[folder] = feature_entropy
        ...
        for folder in self.point_keys:
            ...
            fig, fig_entropy = lidar_points_image(points=pointcloud,ps=ps,test_image=img,cmap="jet")
            dist_lidar[folder] = fig
            dist_lidar_entropy[folder] = fig_entropy
            img_width, img_height = img.size
        ...
        for folder in self.gated_keys:
            ...
            sk_image = imread(file_path) #read the image to convert to skimage ndarray
            entropy_path = "/home/saket/Dense/entropy_sample_gated.png"
            #sk_image = img
            im1 = entropy_image_g(sk_image, entropy_path)
            img = PIL.Image.open(entropy_path, mode="r")
            img = img.resize((1280, 768), Image.ANTIALIAS).convert('RGB')
            img = add_margin(img, 210, 360, 46, 280, (0,0,0))
            # save and load
            img1 = img.save(entropy_path)
            feature_entropy = open(entropy_path, 'rb').read()
            gated_images_entropy[folder] = feature_entropy
        ...
        data['image_data_entropy'] = dist_images_entropy
        data['gated_data_entropy'] = gated_images_entropy
        data['lidar_data_entropy'] = dist_lidar_entropy
        # @ TODO add if needed
    ...
    def create_example(self, data):
        ...
        lidar_data_entropy = data['lidar_data_entropy']
        # radar_data = data['radar_data']
        image_data_entropy = data['image_data_entropy']
        gated_data_entropy = data['gated_data_entropy']
        ...
        for key in self.image_keys:
            ...
            feature_dict['image_entropy/encoded'] = bytes_feature(image_data_entropy[key])
        ...
        for key in self.point_keys:
            ...
            feature_dict['lidar_entropy/encoded'] = bytes_feature(lidar_data_entropy[key])
        ...
        for key in self.gated_keys:
            ...
            feature_dict['gated_entropy/encoded'] = bytes_feature(gated_data_entropy[key])
