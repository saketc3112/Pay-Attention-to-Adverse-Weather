...
def plot_image_projection(pointcloud, vtc, velodyne_to_camera, frame='default', title=None, saveto = "/home/saket/Dense/lidar_proj_2d.png"):
    ...
    if title is not None:
        plt.title(title)
    dpi = 200
    #filename = 'lidar_proj_2d_*.png'
    #cv2.imwrite(filename, lidar_image)
    plt.imshow(lidar_image)
    plt.gca().axes.get_yaxis().set_visible(False)
    plt.gca().axes.get_xaxis().set_visible(False)
    plt.savefig(saveto, dpi=dpi, bbox_inches='tight', pad_inches=0.0)
    #plt.show()
...
class ExampleCreator(object):
    ...
    def process_xmin(self, parsed_labels):
        ...
        for idx,xm in enumerate(parsed_labels.xmin):
            ...
            if parsed_labels.classes[idx] == b'PassengerCar' or parsed_labels.classes[idx] == b'LargeVehicle' or parsed_labels.classes[idx] == b'train':
                xmin_.append(xm)
...
class SwedenImagesv2(ExampleCreator):
    ...
    def read_data(self, entry_id, total_id):
        ...
        for folder in self.point_keys:
            ...
            vtc, velodyne_to_camera = lidar_project_vtc()
            #ps = lidar_project_points(pointcloud, vtc)
            #fig = lidar_points_image(points=pointcloud,ps=ps,test_image=img,cmap="jet")
            lidar_data_last = filter(pointcloud, 1.5)
            fig = plot_image_projection(lidar_data_last, vtc, velodyne_to_camera)
