...
class TfExampleDecoder(data_decoder.DataDecoder):
  ...
  def __init__(self,
    ...
    if dct_method:
      ...
      additional_channel_image = slim_example_decoder.Image(
          ...
          format_key='gated/format',
          channels=1,
          repeated=True)
      # lidar
      lidar = slim_example_decoder.Image(
          image_key='lidar/encoded', format_key='image/format', channels=3)
      additional_channel_image = slim_example_decoder.Image(
          image_key='lidar/additional_channels/encoded',
          format_key='lidar/format',
          channels=1,
          repeated=True)
  ...
  def decode(self, tf_example_string_tensor):
    ...
    tensor_dict[fields.InputDataFields.original_gated_spatial_shape] = tf.shape(
        tensor_dict[fields.InputDataFields.gated])[:2]
    # lidar
    tensor_dict[fields.InputDataFields.lidar].set_shape([None, None, 3])
    tensor_dict[fields.InputDataFields.original_lidar_spatial_shape] = tf.shape(
        tensor_dict[fields.InputDataFields.lidar])[:2]
