...
class SwedenImagesv2(ExampleCreator):
    ...
    def read_data(self, entry_id, total_id):
        ...
        o = self.proces_label(entry_id, dist_images_shape[self.gated_keys[0]])
    ...
    def create_example(self, data):
        ...
        for key in self.gated_keys:
            feature_dict['gated/encoded'] = bytes_feature(image_data[key])
            feature_dict['gated/height'] = int64_feature([x for x in image_height[key]])
            feature_dict['gated/width'] = int64_feature([x for x in image_width[key]])
