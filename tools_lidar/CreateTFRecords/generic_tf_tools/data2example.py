...
class SwedenImagesv2(ExampleCreator):
    ...
    def create_example(self, data):
        ...
        for idx, point_key in enumerate(self.point_keys):
            feature_dict['image/encoded'] = bytes_feature(lidar_data[point_key])
            feature_dict['image/height'] = int64_feature([x for x in image_height[point_key]])
            feature_dict['image/width'] = int64_feature([x for x in image_width[point_key]])
            #feature_dict['image/shape'] = int64_feature([x for x in lidar_data[point_key].shape])
